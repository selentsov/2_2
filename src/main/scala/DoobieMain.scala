...
object DoobieMain {
  def createRoute(tr: Transactor[IO])(implicit ec: ExecutionContextExecutor): Router = {
    ...
    new Router(greetRouter, userRouter)
  }
          ...
          IO.fromFuture(
            IO(
              Http()(system)
                ...
                .bind(createRoute(tr)(system.dispatcher).route)
            )
          )
